#!/usr/bin/env python3
"""
Database initialization script that runs automatically when the container starts.
Checks if database is initialized and initializes it if needed.
This script replaces the manual initialization process.
"""
...
async def check_database_initialized():
    ...
    try:
        ...
        logger.info(f"Database check failed (expected for uninitialized DB): {str(e)}")
        return False
    finally:
        if 'engine' in locals():
        ...
        async with session_factory() as session:
            ...
            roles_data = [
                ...
                },
                {
                    ...
                    "permissions": ["users:read:all", "users:create:all", "users:update:all", "users:delete:all", "analytics:read:all", "sensors:read:all", "sensors:update:all", "sensors:create:all", "sensors:delete:all", "roles:read:all", "roles:create:all", "roles:update:all", "roles:delete:all", "permissions:read:all", "permissions:create:all", "permissions:update:all", "permissions:delete:all", "system:admin:get", "system:admin:post", "system:admin:put", "system:admin:delete", "analytics:export:all", "reports:admin:all"]
                }
            ]
...
async def wait_for_database(timeout=60):
    """Wait for database to be available."""
    import asyncpg

    logger.info(f"Waiting for database to be ready (timeout: {timeout}s)...")

    for i in range(timeout):
        try:
            conn = await asyncpg.connect(
                user=settings.database.url.split('://')[1].split(':')[0],
                password=settings.database.url.split('@')[0].split(':')[-1],
                database=settings.database.url.split('/')[-1].split('?')[0],
                host=settings.database.url.split('@')[1].split(':')[0],
                port=int(settings.database.url.split('@')[1].split(':')[1].split('/')[0])
            )
            await conn.close()
            logger.info("Database is ready!")
            return True
        except Exception:
            logger.debug(f"Database not ready, waiting... ({i+1}/{timeout})")
...
async def main():
    ...
    if await check_database_initialized():
